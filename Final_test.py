...
class AlphaBetaChessTree:
    ...
    def _apply_move(self, move, node, notation="SAN"):
        """
        Applies a chess move to a given game state (node) and prints the board before and after the move.
        ...
        """
        print("Before move:")
        self.print_board(node.board)  # Print the board before the move
        ...
        self.print_board(new_board)  # Print the board after the move
        ...
        node.add_child(new_node, next_turn)  # Add the new node as a child of the current node.
        return new_node
    ...
    def piece_square_score(self, board, color):
        piece_square_table = {
        ...
        for piece_type in piece_square_table:
            for square in board.pieces(piece_type, color):
                piece_position_score = piece_square_table[piece_type][square]
                score += piece_position_score if color == chess.WHITE else -piece_position_score
        return score
    ...
    def _evaluate_position(self, node, depth):
        ...
        # Adding piece-square table scores
        score += self.piece_square_score(board, chess.WHITE) 
        score += self.piece_square_score(board, chess.BLACK)
...
def print_board_text(board):
    ...
    for rank in range(8, 0, -1):
        ...
        for file in range(1, 9):
            piece = board.piece_at(chess.square(file - 1, rank - 1))
            print(symbols.get(piece.symbol(), '.') if piece else '.', end=" ")
        print("|")
    print("  +-----------------+")
    print("    a b c d e f g h  ")
